...
export const renameFile = mutation({
    ...
    handler : async (ctx , args) => {
        ...
        const siblings = await ctx.db.query("files").withIndex("by_project_parent" , q => q.eq("projectId" , project._id).eq("parentId" , file.parentId)).collect();
        const existing = siblings.find(sibling => sibling.name === args.newName && sibling._id !== args.id && sibling.type === file.type)
        if(existing){
