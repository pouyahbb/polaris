...
const createTooltipField = (state : EditorState) : readonly Tooltip[] => {
    ...
    return [
        {
            ...
            create () {
                ...
                dom.className = "bg-popover text-popover-foreground rounded-sm z-50 border border-input p-2 shadow-md flex flex-row gap-2 text-sm"
                
                const addToChatButton = document.createElement("button")
                addToChatButton.textContent = "Add to Chat"
                addToChatButton.className = "font-sans p-1 px-2 hover:bg-foreground/10 rounded-sm whitespace-nowrap"
                
                const quickEditButton = document.createElement("button")
                quickEditButton.className = "font-sans p-1 px-2 hover:bg-foreground/10 rounded-sm flex items-center gap-1.5"
                ...
                quickEditButtonShortcut.className = "text-xs opacity-50"
...
const selectionTooltipField = StateField.define<readonly Tooltip[]>({
    create (state) {
        return createTooltipField(state)
    },
    update(tooltips , transaction) {
        if(transaction.docChanged || transaction.selection){
            return createTooltipField(transaction.state)
        }
