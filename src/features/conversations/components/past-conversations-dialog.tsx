...
const PastConversationsDialog = ({projectId, open, onOpenChange, onSelect}: PastConversationsDialogProps) => {
  ...
  return (
    <CommandDialog open={open} onOpenChange={onOpenChange} title ="Past Conversatios"  description="Search and select a past conversation">
        ...
        <CommandList>
            ...
            <CommandGroup heading="Conversations">
                {conversations?.map(conversation => (
                    <CommandItem key={conversation._id} onSelect={() => handleSelect(conversation._id)} value={`${conversation.title}-${conversation._id}`}>
